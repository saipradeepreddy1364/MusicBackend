...
public class JioSaavnService {
    ...
    /**
     * Fetches and returns the raw audio bytes of a YouTube video.
     * Invokes resolveAudioUrl() (which uses yt-dlp with Piped fallback)
     * and downloads/proxies the bytes.
     */
    public byte[] fetchAudioBytes(String videoId) {
        log.info("fetchAudioBytes | videoId={}", videoId);
        String audioUrl = resolveAudioUrl(videoId);
     ...
     * Uses yt-dlp for reliable extraction, with Piped mirrors as fallback.
     */
        ...
        try {
            ...
            try (java.io.BufferedReader reader = new java.io.BufferedReader(new java.io.InputStreamReader(process.getInputStream()))) {
                ...
                while ((line = reader.readLine()) != null) {
                    if (line.startsWith("https://") && line.contains("googlevideo.com")) {
                        lastUrl = line.trim();
                    }
                }
                process.waitFor(15, java.util.concurrent.TimeUnit.SECONDS);
                if (lastUrl != null) {
                    log.info("resolveAudioUrl | yt-dlp success! url={}", lastUrl);
                    return lastUrl;
                }
            }
        } catch (Exception e) {
            log.error("resolveAudioUrl | yt-dlp failed for videoId={}: {}", videoId, e.getMessage());
        }
        
        // Fallback to Piped API mirror in case yt-dlp is not installed or fails
        log.warn("resolveAudioUrl | Falling back to Piped API mirror for videoId={}", videoId);
